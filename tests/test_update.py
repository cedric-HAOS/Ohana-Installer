...
def test_installer_self_update_downloads_upgrades_and_verifies(
    ...
    release = _installer_release("1.13.1")
    operations: list[str] = []
    ...
    assert operations == [
        "download:ohana_installer-1.13.1-py3-none-any.whl",
        (f"upgrade:ohana_installer-1.13.1-py3-none-any.whl:{sys.executable}"),
    ]
    output = capsys.readouterr().out
    assert "1.13.1 téléchargé et vérifié" in output
    assert "1.13.0 mis à jour" in output
...
def test_installer_self_update_can_be_declined(
    ...
    monkeypatch.setattr(
        update_command,
        "discover_latest_release",
        lambda repository: _installer_release("1.13.1"),
    )
    monkeypatch.setattr(
        update_command,
        "confirm_action",
