...
def test_parse_manifest_rejects_non_wheel_package() -> None:
    ...
    with pytest.raises(
        ManifestError,
        match="doit être égal à 'wheel' ou 'static'",
    ):
        parse_manifest(raw_manifest)


def test_parse_manifest_accepts_static_component() -> None:
    raw_manifest = {
        **VALID_MANIFEST,
        "components": {
            "shizune": {
                ...
                "package": {
                    "type": "static",
                    "filename": "shizune-pwa-0.1.0.tar.gz",
                },
                "static": {"directory": "/var/www/shizune"},
            }
        },
    }
    ...
    assert manifest.components[0].package.type == "static"
    assert manifest.components[0].static is not None
    assert manifest.components[0].static.directory == Path("/var/www/shizune")
...
def test_repository_manifest_is_valid() -> None:
    ...
    assert manifest.platform_name == "Ohana"
    assert manifest.platform_version == "1.0.97"
    ...
    assert agent.version == "1.26.16"
    assert agent.release_tag == "v1.26.16"
    assert agent.package.filename == ("ohana_agent-1.26.16-py3-none-any.whl")
    ...
    assert tuple(
        ...
        ("backup.example.yaml", "plugins/backup.yaml"),
    ...
    assert vision.version == "1.22.13"
    assert vision.release_tag == "v1.22.13"
    assert vision.package.filename == ("ohana_vision-1.22.13-py3-none-any.whl")
    ...
    assert shizune.version == "0.2.3"
    assert shizune.package.type == "static"
    assert shizune.package.filename == "shizune-pwa-0.2.3.tar.gz"
