...
def test_repository_manifest_is_valid() -> None:
    ...
    assert manifest.platform_name == "Ohana"
    assert manifest.platform_version == "1.0.106"
    ...
    assert agent.version == "1.29.7"
    assert agent.release_tag == "v1.29.7"
    assert agent.package.filename == ("ohana_agent-1.29.7-py3-none-any.whl")
    ...
    assert shizune.version == "0.3.0"
    assert shizune.package.type == "static"
    assert shizune.package.filename == "shizune-pwa-0.3.0.tar.gz"
